	...
	return (
					...
					<Component.Card.Detail.Aside title="Exhibitor Information">
						{data.exhibition.info.map(detail => (
							<Box
								...
								style={theme => ({
									borderBottom:
										data.exhibition.info.indexOf(detail) != data.exhibition.info.length - 1 &&
										`2px solid ${theme.colors.pri[6]}`,
									padding: `${theme.spacing.md} 0`,
								})}
							>
								<Component.Card.Detail.Item data={detail} />
							</Box>
						))}
		...
		>
			<Layout.Page>
				...
				{/* <Layout.Section id="details" bg="white">
					<Container size={"lg"} w={"100%"}>
						<Stack gap={"xl"}>
									...
									<Component.Card.Detail.Aside title="Exhibitor Information">
										{data.exhibition.info.map(detail => (
											<Box
												...
												style={theme => ({
													borderBottom:
														data.exhibition.info.indexOf(detail) !=
															data.exhibition.info.length - 1 &&
														`2px solid ${theme.colors.pri[6]}`,
													padding: `${theme.spacing.md} 0`,
												})}
											>
							...
							<Text component="p" fz={"sm"} c={"sec"} ta={"center"}>
								<Text component="span" fz={"inherit"} c={"pri"} fw={500}>
									Note
								</Text>
								: The Exhibition Hall and Conference Room will be locked everyday after the event and no
								items will be removed without authorization from the exhibitor concerned, organizers or
								the management of Sarit Expo Centre.
							</Text>
						</Stack>
					</Container>
				</Layout.Section> */}

				{/* <Partial.Cta.Expected image={image.gallery.expo.yr2022.image9} />
